...
export const addressHandler = async ({
    ...
    try {
        // Geocoder request to fetch address data
        ...
        json.results[0].address_components.map((elem) => {
            if (
                ...
                addressObj = { ...addressObj, city: elem.long_name };
            } else if (elem.types[0] === 'administrative_area_level_1') {
                addressObj = { ...addressObj, state: elem.long_name };
            } else if (elem.types[0] === 'country') {
                addressObj = { ...addressObj, country: elem.long_name };
            } else if (elem.types[0] === 'postal_code') {
                addressObj = { ...addressObj, zipCode: elem.long_name };
            }
        });
